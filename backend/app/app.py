...
async def search_photographers(request: SearchRequest):
    ...
    try:
        ...
        logger.info(f"Request: destination='{request.destination}', language='{request.preferred_language}'")
        ...
        raise HTTPException(
            status_code=500,
            detail={"error": "Internal Server Error", "message": str(e), "traceback": traceback.format_exc()}
        )
